...
def main():
    while True:
        try:
            ...
            user_input = input("Nhập 1 để chạy tiếp: ").strip()
            if user_input == "1":
                ...
                # upload video
                upload_yt(
                    "C:/Path/To/Chrome/trade-yt",
                    title,
                    description,
                    tags,
                    os.path.abspath(f'./videos/{title_slug}.mp4'),
                    os.path.abspath(folder_path + f'/picture{reason_contents.__len__() + 3}.png'),
                )
